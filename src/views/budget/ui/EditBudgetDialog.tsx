...
import { updateBudget, type Budget } from "@/entities/budget";
...
// Rendered only when budget is non-null; caller passes key={budget.id} to reset state on budget change.
export function EditBudgetDialog({ budget, onClose, onUpdated }: EditBudgetDialogProps) {
  const titleId = useId();

  const [title, setTitle] = useState(budget.title);
  const [totalAmount, setTotalAmount] = useState(String(budget.totalAmount));
  const [usedAmount, setUsedAmount] = useState(String(budget.usedAmount));
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <p className="mt-1 text-sm text-zinc-500">예산 정보를 수정합니다.</p>
