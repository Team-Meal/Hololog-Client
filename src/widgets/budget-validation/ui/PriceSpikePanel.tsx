import type { SubstitutionSaving } from "@/entities/price";

interface Props {
  substitutions: SubstitutionSaving[];
}

export function PriceSpikePanel({ substitutions }: Props) {
      ...
      {substitutions.map((s) => (
        <div key={s.itemName} className="text-sm">
          <p className="text-amber-700">
            가격 상승 품목: <span className="font-semibold">{s.itemName}</span> (KAMIS 기준 전주
            대비 급등)
          </p>
