...
export function OrderCard({ item, actions }: { item: OrderItem; actions?: ReactNode }) {
  return (
    <article className="rounded-[24px] bg-white p-5 shadow-[0_1px_0_rgba(15,23,42,0.02)] transition-all duration-200 ease-out hover:-translate-y-0.5 hover:shadow-sm">
      <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[minmax(0,1.4fr)_repeat(3,minmax(0,1fr))]">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <h2 className="truncate text-lg font-bold text-gray-950">{item.name}</h2>
        ...
        <div className="flex flex-col items-start gap-1.5 lg:items-end">
          <p className="text-xs font-semibold text-gray-400">권장 구매량</p>
      ...
      <div className="mt-4 flex items-start gap-2.5 rounded-2xl bg-blue-50/60 px-4 py-3">
        ...
        <p className="text-sm font-medium text-gray-600">{item.note}</p>
      </div>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="mt-1.5 text-lg font-bold text-gray-950">{value}</p>
    </div>
  );
}
